...
static GPUMemory<float> load_image_gpu_rgba(const std::string &filename, int &width, int &height)
{
...
static int choose_cuda_device_for_gl_context()
{
    ...
    int cuda_devices[8]{};
    cudaError_t err = cudaGLGetDevices(&cuda_device_count, cuda_devices, 8, cudaGLDeviceListCurrentFrame);
...
int main(int argc, char **argv)
{
    ...
    RGFW_windowFlags winflags = RGFW_windowAllowDND | RGFW_windowCenter | RGFW_windowScaleToMonitor | RGFW_windowOpenGL;
    RGFW_window *window = RGFW_createWindow("Neural-SDF", SCR_WIDTH, SCR_HEIGHT, SCR_WIDTH, SCR_HEIGHT, winflags);
    if (window == NULL)
    {
    ...
    CUDA_CHECK_THROW(cudaCreateTextureObject(&image_tex, &resDesc, &texDesc, nullptr));
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, imgW, imgH, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
    ...
    auto reload_image = [&](const std::string &image_path)
    {
        ...
        try
        {
            int new_img_w = 0;
            int new_img_h = 0;
            GPUMemory<float> new_image = load_image_gpu_rgba(image_path, new_img_w, new_img_h);

            cudaResourceDesc new_res_desc{};
            new_res_desc.resType = cudaResourceTypePitch2D;
            new_res_desc.res.pitch2D.devPtr = new_image.data();
            new_res_desc.res.pitch2D.desc = cudaCreateChannelDesc(32, 32, 32, 32, cudaChannelFormatKindFloat);
            ...
            CUDA_CHECK_THROW(cudaCreateTextureObject(&new_image_tex, &new_res_desc, &texDesc, nullptr));

            size_t new_frame_bytes = (size_t)new_img_w * (size_t)new_img_h * sizeof(uchar4);
            glGenBuffers(1, &new_pbo);
            glBindBuffer(GL_PIXEL_UNPACK_BUFFER, new_pbo);
            glBufferData(GL_PIXEL_UNPACK_BUFFER, new_frame_bytes, nullptr, GL_STREAM_DRAW);
            glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);

            CUDA_CHECK_THROW(cudaGraphicsGLRegisterBuffer(&new_cuda_pbo_res, new_pbo, cudaGraphicsRegisterFlagsWriteDiscard));

            uint32_t new_n_coords = (uint32_t)(new_img_w * new_img_h);
            uint32_t new_n_coords_padded = next_multiple(new_n_coords, BATCH_SIZE_GRANULARITY);
            ...
            GPUMatrix<float> new_inference_batch(new_xs_and_ys.data(), n_input_dims, new_n_coords_padded);
    ...
    while (!RGFW_window_shouldClose(window) && running)
    {
        ...
        if (training_enabled)
        {
            ...
            generate_random_uniform<float>(stream, rng, batch_size * n_input_dims, training_batch.data());
            linear_kernel(eval_image<n_output_dims>, 0, stream, batch_size, image_tex, training_batch.data(), training_target.data());
        ...
        size_t pbo_bytes = 0;
        CUDA_CHECK_THROW(cudaGraphicsResourceGetMappedPointer(&pbo_dev_ptr, &pbo_bytes, cuda_pbo_res));

        if (pbo_bytes < frame_bytes)
        {
            fprintf(stderr, "Mapped PBO (%zu) smaller than frame (%zu).\n", pbo_bytes, frame_bytes);
        ...
        pack_rgba8_from_prediction<<<blocks, threads, 0, stream>>>(imgW, imgH, prediction.data(), (int)n_output_dims, reinterpret_cast<uchar4 *>(pbo_dev_ptr));
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, imgW, imgH, GL_RGBA, GL_UNSIGNED_BYTE, (void *)0);
        glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
        glBindTexture(GL_TEXTURE_2D, 0);
        ...
        int loss_plot_offset = loss_history_count == kLossHistorySize ? loss_history_offset : 0;

        ImGui::SetNextWindowPos(ImVec2(12.0f, 12.0f), ImGuiCond_Once);
        ImGui::SetNextWindowSize(ImVec2(420.0f, 180.0f), ImGuiCond_Once);
        ...
        ImGui::Text("Image: %s", std::filesystem::path(activeImagePath).filename().string());
        ImGui::Text("Iteration: %u", current_iteration);
        ImGui::Text("Loss: %.6f", current_loss);
        ImGui::PlotLines("Loss", loss_history.data(), loss_history_count, loss_plot_offset, nullptr, loss_min, loss_max, ImVec2(0.0f, 90.0f));
        ImGui::End();
